...
func (r *RouteGuideService) GetFeature(ctx context.Context, point *routeGuide.Point) (*routeGuide.Feature, error) {
...
func (r *RouteGuideService) ListFeatures(rect *routeGuide.Rectangle, stream routeGuide.RouteGuide_ListFeaturesServer) error {
	for _, feature := range r.savedFeatures {
...
// total time spent.
func (r *RouteGuideService) RecordRoute(stream routeGuide.RouteGuide_RecordRouteServer) error {
...
func (r *RouteGuideService) RouteChat(stream routeGuide.RouteGuide_RouteChatServer) error {
	for {
		in, err := stream.Recv()
		if err == io.EOF {
			return nil
		...
		rn := make([]*routeGuide.RouteNote, len(r.routeNotes[key]))
...
// RouteChat receives a stream of message/location pairs, and responds with a stream of all
// previous messages at each of those locations.
func (r *RouteGuideService) loadFeatures() {
...
func NewRouteGuideServer() *RouteGuideService {
	s := &RouteGuideService{routeNotes: make(map[string][]*routeGuide.RouteNote)}
	s.loadFeatures()
	return s
}
